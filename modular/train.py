...
def create_model(model_choice, optimizer_choice, base_model, schedule_lr):
    # output size
    ...
    if optimizer_choice == 'adam':
        optimizer = optim.Adam(model.parameters(), lr=setup_hyperparameters.LEARNING_RATE)
    elif optimizer_choice == 'adamw':
        optimizer = optim.AdamW(model.parameters(), lr=setup_hyperparameters.LEARNING_RATE)
    ...
    if schedule_lr:
        if optimizer_choice in ['sgd_scheduled', 'adam', 'adamw']:
            scheduler = lr_scheduler.StepLR(optimizer, step_size=10, gamma=0.1)
            print("Using StepLR scheduler")
        elif optimizer_choice in ['rmsprop']:
...
def classifier(model_path, model_choice, base_model, schedule_lr, optimizer_choice, optimizer):
...
def inference_for_truck_1(img_dir, out_dir, model_path, base_model, model_choice, schedule_lr, optimizer_choice, optimizer):
    ...
    #
    # # Load the model
    model = classifier(model_path, model_choice, base_model, schedule_lr, optimizer_choice, optimizer)
    model.to(device)
...
def data_set_up(transform=None):
...
def train_model(transform, model_choice, base_model, version_model, schedule_lr, optimizer_choice):
    min_loss = 999
    results_best = dict()
    ratio_best = 0

    for ratio in freeze_ratios:
        model, optimizer, loss_fn, scheduler = create_model(model_choice, optimizer_choice, base_model, schedule_lr)
